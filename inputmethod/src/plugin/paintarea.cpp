...
{
    image=QImage(this->size(),QImage::Format_RGB32);   //画布的初始化大小设为400*300，使用32位颜色
...
{
    if(event->buttons()&Qt::LeftButton)   //如果鼠标左键按着同时移动鼠标
    {
...
bool PaintArea::saveImage(const QString &fileName, const char *fileFormat)
{
    ...
    if(visibleImage.save(fileName,fileFormat))  //已经实现了存储
    {
        modified=false;
        return true;
    }
