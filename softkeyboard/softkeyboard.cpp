...
#define PINYINFILEPATH  ". ./softkeyboard-master/ChinesePinyin"
...
void SoftKeyboard::readDictionary()
{
    ...
    QFileInfo fileInfo(PINYINFILEPATH);
    QString absPath = fileInfo.absoluteFilePath();
    qDebug() << "文件的绝对路径是：" << absPath;
    if (QFile::exists(PINYINFILEPATH)) {
       qDebug()<<QDateTime::currentDateTime().toString("yyyy-MM-dd HH:m:s:z") + "11111111";
    } else {
        qDebug()<<QDateTime::currentDateTime().toString("yyyy-MM-dd HH:m:s:z") + "222222222";
    }
    if(!pinyinFile.open(QIODevice::ReadOnly))//拼音文件打开失败提示
    {

        qDebug()<<PINYINFILEPATH;
